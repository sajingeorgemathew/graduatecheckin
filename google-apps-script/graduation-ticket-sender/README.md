...
| Key | Default | Notes |
| --- | --- | --- |
| `TEST_MODE` | `TRUE` | While `TRUE`, all mail goes to `TEST_RECIPIENT_EMAIL`. |
...
| `PRODUCTION_CONFIRMATION` | *(blank)* | Must equal `SEND CONVOCATION 2026 TICKETS` to send production. Cleared after each run. |
...
3. Set `PRODUCTION_CONFIRMATION = SEND CONVOCATION 2026 TICKETS`.
